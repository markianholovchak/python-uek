...
while(counter < numOfPrimes):
    ...
    Look for prime numbers until enough
     - Check if the checked number is divisible by some other integer between 2 and itself
     - If it is, then it is not a prime number and we move to a number greater by 1
     - If the number is not divisible by any other integer than 2 or itself then append the number
